...
# Individual component EPA functions for 2025
def l1_epa_2025(breakdowns, team_count):
    """Calculate L1 (trough) EPA for 2025 matches."""
    def score_per_breakdown(b):
        reef = b.get("autoReef", {})
        auto_trough = reef.get("trough", 0) * 3  # Auto trough is worth 3 points
        
        reef_teleop = b.get("teleopReef", {})
        teleop_trough = reef_teleop.get("trough", 0) * 2  # Teleop trough is worth 2 points
        
        total_trough = auto_trough + teleop_trough
        
        # Scale based on alliance size
        scaling_factor = 1 / (1 + math.log(team_count)) if team_count > 1 else 1.0
        return total_trough * scaling_factor
...
def l2_epa_2025(breakdowns, team_count):
    """Calculate L2 (bot row) EPA for 2025 matches."""
    def score_per_breakdown(b):
        reef = b.get("autoReef", {})
        auto_bot = reef.get("tba_botRowCount", 0) * 4  # Auto bot row is worth 4 points
        
        reef_teleop = b.get("teleopReef", {})
        teleop_bot = reef_teleop.get("tba_botRowCount", 0) * 3  # Teleop bot row is worth 3 points
        
        total_bot = auto_bot + teleop_bot
        
        # Scale based on alliance size
        scaling_factor = 1 / (1 + math.log(team_count)) if team_count > 1 else 1.0
        return total_bot * scaling_factor
...
def l3_epa_2025(breakdowns, team_count):
    """Calculate L3 (mid row) EPA for 2025 matches."""
    def score_per_breakdown(b):
        reef = b.get("autoReef", {})
        auto_mid = reef.get("tba_midRowCount", 0) * 6  # Auto mid row is worth 6 points
        
        reef_teleop = b.get("teleopReef", {})
        teleop_mid = reef_teleop.get("tba_midRowCount", 0) * 4  # Teleop mid row is worth 4 points
        
        total_mid = auto_mid + teleop_mid
        
        # Scale based on alliance size
        scaling_factor = 1 / (1 + math.log(team_count)) if team_count > 1 else 1.0
        return total_mid * scaling_factor
...
def l4_epa_2025(breakdowns, team_count):
    """Calculate L4 (top row) EPA for 2025 matches."""
    def score_per_breakdown(b):
        reef = b.get("autoReef", {})
        auto_top = reef.get("tba_topRowCount", 0) * 7  # Auto top row is worth 7 points
        
        reef_teleop = b.get("teleopReef", {})
        teleop_top = reef_teleop.get("tba_topRowCount", 0) * 5  # Teleop top row is worth 5 points
        
        total_top = auto_top + teleop_top
        
        # Scale based on alliance size
        scaling_factor = 1 / (1 + math.log(team_count)) if team_count > 1 else 1.0
        return total_top * scaling_factor
...
def processor_epa_2025(breakdowns, team_count):
    """Calculate processor (wall algae) EPA for 2025 matches."""
    def score_per_breakdown(b):
        processor = b.get("wallAlgaeCount", 0) * 2.5  # Processor is worth 2.5 points
        
        # Scale based on alliance size
        scaling_factor = 1 / (1 + math.log(team_count)) if team_count > 1 else 1.0
        return processor * scaling_factor
...
def net_epa_2025(breakdowns, team_count):
    """Calculate net (net algae) EPA for 2025 matches."""
    def score_per_breakdown(b):
        net = b.get("netAlgaeCount", 0) * 4  # Net is worth 4 points
        
        # Scale based on alliance size
        scaling_factor = 1 / (1 + math.log(team_count)) if team_count > 1 else 1.0
        return net * scaling_factor
