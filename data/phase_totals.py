...
def robot_endgame_2026(b: dict, index: int) -> float:
    """Tower climb only. Hub endgame fuel is intentionally left in teleop."""
...
def _per_robot_ends(year: int, b: dict, team_count: int) -> Optional[List[float]]:
    ...
    if y == 2015:
        return [0.0] * n
    if y == 2026:
        # Tower climbs only. Hub endgame fuel stays in teleop via total - auto - end.
...
def alliance_auto(year: int, breakdown: dict, team_count: int = 3) -> float:
    ...
    if auto or breakdown.get("autoPoints") is not None or breakdown.get("auto_points") is not None:
        return auto
    # Fallback used by the locked 2015–2026 recompute (yearmodels path for 2026).
