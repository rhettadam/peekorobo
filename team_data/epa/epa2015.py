...
def load_veteran_teams():
    try:
        with open("teams_2014.json", "r") as f:
            data = json.load(f)
...
def calculate_epa_components_2015(matches, team_key, year, team_epa_cache=None, veteran_teams=None):
    import statistics
    ...
    total_score = wins = losses = 0  # 2015 had no "wins" technically but this still counts if "winning_alliance" is present
    ...
    for match in matches:
        ...
        # Score components
        actual_auto = breakdown.get("auto_points", 0)
        actual_tote = breakdown.get("tote_points", 0) / team_count
        actual_container = breakdown.get("container_points", 0) / team_count
        actual_litter = breakdown.get("litter_points", 0) / team_count

        actual_teleop = actual_tote + actual_container + actual_litter
        actual_endgame = 0  # Not meaningful in 2015
        actual_overall = actual_auto + actual_teleop
    ...
    return {
        ...
        "endgame": round(endgame_epa, 2),  # 0 always
        "trend": round(trend, 2),
...
def fetch_team_components(team, year, team_epa_cache=None, veteran_teams=None):
    ...
    try:
        ...
        components = calculate_epa_components_2015(matches, team_key, year, team_epa_cache, veteran_teams) if matches else None
...
def fetch_and_store_team_data():
    for year in tqdm(range(2015,2016), desc="Processing Years"):
        ...
        def fetch_epa_for_cache(team):
            ...
            try:
                matches = tba_get(f"team/{team_key}/matches/{year}")
                components = calculate_epa_components_2015(matches, team_key, year, None, veteran_teams)
                if components:
