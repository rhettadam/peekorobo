...
def load_veteran_teams():
    try:
        with open("teams_2020.json", "r") as f:
            data = json.load(f)
...
def calculate_epa_components_2021(matches, team_key, year, team_epa_cache=None, veteran_teams=None):
    import statistics
    ...
    match_count = 0
    overall_epa = auto_epa = teleop_epa = endgame_epa = 0
    trend_deltas, contributions, teammate_epas = [], [], []
    total_score = wins = losses = 0

    for match in matches:
        if team_key not in match.get("alliances", {}).get("red", {}).get("team_keys", []) and \
           team_key not in match.get("alliances", {}).get("blue", {}).get("team_keys", []):
            continue
        ...
        # No reliable breakdown structure in 2021; estimate components
        actual_auto = breakdown.get("auto_points", 0) / team_count
        actual_teleop = breakdown.get("teleop_points", 0) / team_count
        actual_endgame = 0  # Not meaningful in 2021 at-home
        actual_overall = (breakdown.get("total_points", 0)) / team_count
        ...
        delta_endgame = 0  # intentionally ignored
...
def fetch_team_components(team, year, team_epa_cache=None, veteran_teams=None):
    ...
    try:
        ...
        components = calculate_epa_components_2021(matches, team_key, year, team_epa_cache, veteran_teams) if matches else None
...
def fetch_and_store_team_data():
    for year in tqdm(range(2021, 2022), desc="Processing Years"):
        ...
        def fetch_epa_for_cache(team):
            ...
            try:
                matches = tba_get(f"team/{team_key}/matches/{year}")
                components = calculate_epa_components_2021(matches, team_key, year, None, veteran_teams)
                if components:
