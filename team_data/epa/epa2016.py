...
def load_veteran_teams():
    try:
        with open("teams_2015.json", "r") as f:
            data = json.load(f)
...
def calculate_epa_components_2016(matches, team_key, year, team_epa_cache=None, veteran_teams=None):
    import statistics
    ...
    for match in matches:
        ...
        # === EPA COMPONENTS ===
        # Auto
        auto_boulders = breakdown.get("autoBoulderPoints", 0)
        auto_crossing = breakdown.get("autoCrossingPoints", 0)
        actual_auto = (auto_boulders + auto_crossing) / team_count

        # Teleop
        teleop_boulders = breakdown.get("teleopBoulderPoints", 0)
        teleop_crossing = breakdown.get("teleopCrossingPoints", 0)
        actual_teleop = (teleop_boulders + teleop_crossing) / team_count

        # Endgame
        endgame_challenge = breakdown.get("teleopChallengePoints", 0)
        endgame_scale = breakdown.get("teleopScalePoints", 0)
        actual_endgame = (endgame_challenge + endgame_scale) / team_count

        # Bonus points (treated like consistency boosters)
        if breakdown.get("breachPoints", 0):
            actual_teleop += 20 / team_count
        if breakdown.get("capturePoints", 0):
            actual_endgame += 25 / team_count
...
def fetch_team_components(team, year, team_epa_cache=None, veteran_teams=None):
    ...
    try:
        ...
        components = calculate_epa_components_2016(matches, team_key, year, team_epa_cache, veteran_teams) if matches else None
...
def fetch_and_store_team_data():
    for year in tqdm(range(2016,2017), desc="Processing Years"):
        ...
        def fetch_epa_for_cache(team):
            ...
            try:
                matches = tba_get(f"team/{team_key}/matches/{year}")
                components = calculate_epa_components_2016(matches, team_key, year, None, veteran_teams)
                if components:
