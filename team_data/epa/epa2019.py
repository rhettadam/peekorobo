...
def load_veteran_teams():
    try:
        with open("teams_2018.json", "r") as f:
            data = json.load(f)
        return {f"frc{team['team_number']}" for team in data if 'team_number' in team}
    except FileNotFoundError:
        print("Warning: teams_2018.json not found. All teams will be treated as rookies.")
        return set()
...
def calculate_epa_components_2019(matches, team_key, year, team_epa_cache=None, veteran_teams=None):
    ...
    for match in matches:
        ...
        # === AUTO ===
        auto_points = breakdown.get("sandStormBonusPoints", 0)
        actual_auto = auto_points / team_count

        # === TELEOP ===
        teleop_points = breakdown.get("teleopPoints", 0) - breakdown.get("habClimbPoints", 0)
        actual_teleop = teleop_points / team_count

        # === ENDGAME ===
        actual_endgame = breakdown.get("habClimbPoints", 0) / team_count
