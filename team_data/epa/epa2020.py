...
def load_veteran_teams():
    try:
        with open("teams_2019.json", "r") as f:
            data = json.load(f)
        return {f"frc{team['team_number']}" for team in data if 'team_number' in team}
    except FileNotFoundError:
        print("Warning: teams_2019.json not found. All teams will be treated as rookies.")
        return set()

def calculate_epa_components_2020(matches, team_key, year, team_epa_cache=None, veteran_teams=None):
    import statistics
    ...
    for match in matches:
        ...
        # === TELEOP ===
        teleop_points = breakdown.get("teleopPoints", 0) - breakdown.get("endgamePoints", 0)
...
def fetch_team_components(team, year, team_epa_cache=None, veteran_teams=None):
    ...
    try:
        ...
        components = calculate_epa_components_2020(matches, team_key, year, team_epa_cache, veteran_teams) if matches else None
...
def fetch_and_store_team_data():
    for year in tqdm(range(2020, 2021), desc="Processing Years"):
        ...
        def fetch_epa_for_cache(team):
            ...
            try:
                matches = tba_get(f"team/{team_key}/matches/{year}")
                components = calculate_epa_components_2020(matches, team_key, year, None, veteran_teams)
                if components:
