...
def load_veteran_teams():
    try:
        with open("teams_2016.json", "r") as f:
            data = json.load(f)
...
def calculate_epa_components_2017(matches, team_key, year, team_epa_cache=None, veteran_teams=None):
    import statistics
    ...
    for match in matches:
        ...
        # === AUTO ===
        auto_points = (
            breakdown.get("autoMobilityPoints", 0) +
            breakdown.get("autoRotorPoints", 0) +
            breakdown.get("autoFuelPoints", 0)
        )
        actual_auto = auto_points / team_count

        # === TELEOP ===
        teleop_points = (
            breakdown.get("teleopRotorPoints", 0) +
            breakdown.get("teleopFuelPoints", 0)
        )
        actual_teleop = teleop_points / team_count

        # === ENDGAME ===
        actual_endgame = breakdown.get("teleopTakeoffPoints", 0) / team_count
...
def fetch_team_components(team, year, team_epa_cache=None, veteran_teams=None):
    ...
    try:
        ...
        components = calculate_epa_components_2017(matches, team_key, year, team_epa_cache, veteran_teams) if matches else None
...
def fetch_and_store_team_data():
    for year in tqdm(range(2017, 2018), desc="Processing Years"):
        ...
        def fetch_epa_for_cache(team):
            ...
            try:
                matches = tba_get(f"team/{team_key}/matches/{year}")
                components = calculate_epa_components_2017(matches, team_key, year, None, veteran_teams)
                if components:
